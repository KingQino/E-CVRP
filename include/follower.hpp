...
public:
    ...
    void clean();                                    // clean the follower data structures
    ...
    void refine(Individual* ind);                    // refine the final solution by enumerate all possible charging visits
    ...
    ChargingMeta try_enumerate_n_stations_to_route(int m_len, int n_len, int* chosen_sta, int* chosen_pos, double& cost,
        int cur_upper_bound, const int* route, int length, const vector<double>& accumulated_distance) const;
